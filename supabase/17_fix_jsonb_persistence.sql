-- [17] FIX PERSISTÊNCIA DE JSONB (COMENTÁRIOS E ANEXOS)
-- Adiciona p_comments e p_attachments às funções atômicas para garantir salvamento seguro e logado.
...
BEGIN
    ...
    IF v_user_role NOT IN ('admin', 'desenvolvedor', 'user') THEN
        RAISE EXCEPTION 'Permissão negada. Perfil (%) não autorizado a editar pautas.', v_user_role;
    END IF;
    ...
    VALUES (v_task.id, auth.uid(), COALESCE(v_user_name, 'Sistema'), 'edit', 'Pauta atualizada via RPC Segura (v2 - JSONB Fixed).');
...
BEGIN
    ...
    IF v_user_role NOT IN ('admin', 'desenvolvedor', 'user') THEN
        RAISE EXCEPTION 'Permissão negada. Perfil (%) não autorizado a criar pautas.', v_user_role;
    END IF;
    ...
    INSERT INTO public.task_logs (task_id, user_id, user_name, action_type, details)
    VALUES (v_task.id, auth.uid(), COALESCE(v_user_name, 'Sistema'), 'create', 'Pauta criada com sucesso (v2 - JSONB Fixed).');
