...
-- 2. CRIAR A FUNÇÃO DE LOG DETECTIVE (PROFISSIONAL + MONITORAMENTO DE FOTOS)
...
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        ...
        -- NOVO: MONITORAMENTO DE ANEXOS/FOTOS
        IF (OLD.attachments IS DISTINCT FROM NEW.attachments) THEN
            IF (jsonb_array_length(NEW.attachments) > jsonb_array_length(OLD.attachments)) THEN
                detail_msg := detail_msg || 'Anexou novas fotos/arquivos. ';
            ELSIF (jsonb_array_length(NEW.attachments) < jsonb_array_length(OLD.attachments)) THEN
