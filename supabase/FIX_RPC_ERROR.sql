-- SCRIPT DE REPARAÇÃO: Limpar funções duplicadas (Overloading) no PostgreSQL

-- 1. DELETA TODAS AS VERSÕES DA FUNÇÃO create_task_atomic (Antiga e Nova)
DO $$ 
DECLARE
    r record;
BEGIN
    FOR r IN SELECT oid::regprocedure AS fname 
             FROM pg_proc 
             WHERE proname = 'create_task_atomic'
    LOOP
        EXECUTE 'DROP FUNCTION ' || r.fname || ' CASCADE;';
    END LOOP;
END $$;

-- 2. RECRIAR A FUNÇÃO CORRETA COM OS CAMPOS DE POSTAGEM
