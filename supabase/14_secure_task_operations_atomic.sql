...
-- [SCENARIO 14] ENDURECIMENTO DE SEGURANÇA (HARDENING)
-- Este script redefine as funções de criação e edição para incluir verificação interna de papéis (Role Gate).
...
BEGIN
    ...
    VALUES (v_task.id, auth.uid(), COALESCE(v_user_name, 'Sistema'), 'create', 'Pauta criada com sucesso.');
...
BEGIN
    ...
    INSERT INTO public.task_logs (task_id, user_id, user_name, action_type, details)
    VALUES (v_task.id, auth.uid(), COALESCE(v_user_name, 'Sistema'), 'edit', 'Pauta atualizada via RPC Segura.');
