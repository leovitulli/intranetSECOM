...
async function fetchPage(pageNum) {
    ...
    // FIX: [^>]* instead of href=".*?" to handle hreflang attribute
...
async function main() {
    console.log('🚀 Radar Notícias — RE-Sync (regex corrigido)\n');
    ...
    for (let p = 0; p < totalPages; p++) {
        try {
            ...
            news.forEach(n => { if (n.category) categoriesSeen.add(n.category); });
            ...
            if ((p + 1) % 10 === 0 || p === totalPages - 1) {
                ...
                console.log(`  [${p + 1}/${totalPages}] ${pct}% — ${grandTotal} itens — ${categoriesSeen.size} categorias`);
            }
            await delay(200);
        } catch (err) {
    ...
    console.log(`\n✅ Re-Sync Complete! ${grandTotal} parseados, ${count} no banco, ${totalErrors} erros`);
    console.log(`📂 Categorias encontradas: ${Array.from(categoriesSeen).sort().join(', ')}`);
}
